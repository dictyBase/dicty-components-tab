...
const TabBar = styled.div`
  ...
  @media (max-width: 917px) {
    ...
    display: flex;
    max-width: 100%;
    ${"" /* border-top: 1px solid black; */} -ms-overflow-style: none;
...
export default class Tabs extends Component<Props, State> {
      ...
      return (
        <Tab key={i} active={active}>
          <Link to={`${match.url}/${tab.link}`} active={active}>
            <Text active={active}>{tab.title}</Text>
          </Link>
        </Tab>
      )
    })
  ...
  handleScroll = (): void => {
    ...
    if (this.tabBar.scrollLeft < 15 && scrollPos != "left") {
      ...
      this.tabBar.scrollLeft > 15 &&
      this.tabBar.scrollLeft < max - 15
    ) {
      this.setState({
        scrollPos: "middle",
      })
      ...
      <ThemeProvider theme={theme}>
        <Container>
          ...
          <TabBar innerRef={el => (this.tabBar = el)}>
            <LeftArrow
              visible={
                scrollPos != "left" && scrollPos != "none" ? true : false
              }
              pos="left"
            />
            {this.renderTabs()}
            <RightArrow
              visible={
                scrollPos != "right" && scrollPos != "none" ? true : false
              }
              pos="right"
            />
          </TabBar>
          <Content innerRef={el => (this.content = el)}>
            <Route
              path={`${match.url}/:tab`}
              component={props => <Pane {...props} tabs={tabs} />}
            />
          </Content>
        </Container>
      </ThemeProvider>
